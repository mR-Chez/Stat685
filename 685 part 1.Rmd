...
Using the parameter `color = Subject` within the `aes()` call, we can color each data point by which Subject the measurement came from. For example, the legend shows that Subject 309 is depicted by a deep orange dot:
...
1. The $e_{ij}$'s are independent with mean equal to 0, finite variance, and follow $Normal(0,\sigma^2_e )$  
2. The $C_1, C_2, ..., C_n$ are all independent and follow $Normal(0,\sigma^2_c)$   
3. The $e_{ij}$ and $C_i$ are independent    
4. The $e_{ij}$, $C_i$, and $t_j$ are all independent   
...
We need to first load in the *rstan* and *brms* packages which will allows us to build Bayesian models. Then we will use estimates from a mixed model built with the `lme4` package for priors. See the `lme4` section of this manual for more info.
...
b <- summary(lmm)$coefficients[1,2]
...
**Chains**: The number of Markov chains to use. The advantage of using more chains is that we can be more confident of our understanding of the posterior distribution when more chains are being used. This packages allows the user to choose up to the number of cores your system has - defaults to 4.
...
More iterations took longer to fit. If a model fails to converge, you will see a warning output and may need to adjust iterations or other fitting options in order to be successful. Experiment with iterations until you reach an acceptable output. **Note**: If you want to compare fitting times for different iterations, un-comment the code block above this section in the markdown file for an example comparison.
...
The first 5 lines of the summary output describe our model and model inputs defined in the `brm` call. The next section shows the Group-Level effects. Our estimate for the subject-specific random effect is `r summary(model1)$random$Subject$Estimate`, with Est.Error of `r summary(model1)$random$Subject$Est.Error` which represents the standard deviation of the distribution of the posterior. 

If you want to access a specific estimate from the model object, they can be accessed by using the `$` operator on the summary object of the model. For example: `summary(model1)$random$Subject` returns Subject-specific random effects:

```{r, echo=FALSE, message = FALSE}
summary(model1)$random$Subject
```

The `$` notation can be used in addition to the last command to access any part of this output.
...
This section shows our intercept estimate, $\hat\beta_0$ = 204.98, our slope estimate, $\hat\beta$ = 10.28 along with credible intervals, $\hat R$, and bulk/tail ESS for each.
