...
output: 
  html_document:
    ...
    toc_depth: 5 # how deep should the toc go
    toc_float: true # keeps TOC on the left side when scrollling
    collapsed: false 
---
```{r install packages, echo=F, message=FALSE, eval=FALSE}
# Packages needed for this script to run
install.packages(c("knitr", "rstan", "brms", "tidyverse", "lme4"))

# If running on Windows, "RTools" package may be required:
install.packages("RTools")

# If running R 4.2.1+, RTools42 may be required:
...
```
**Notes**: 
* Within `prior()` calls, sigma refers to the population-level and sd refers to the group-level
* The `brms` package adjustes prior distributions to be greater than 0 in `sd` and `sigma` priors.
* To compare these results with the output from a model built with the `lme4` package, please see the frequentist section of this manual above.
...
* If you want to compare fitting times for different iterations, refer to the "iteration_comparison" section of accompanying code this markdown file.
...
Reaction time $y_{ij}$ is equal to population intercept $\beta_0$ plus population slope $\beta_1 t$ times the number of days $t_j$, plus subject specific random intercept $C_{i,0}$ plus subject specific random slope $C_{i,1}$ times the number of days $t_j$, plus an error term $e_{ij}$. $C_{i,0}$ and $C_{i,1}$ are both independent of $e_{ij}$. 

Additional Assumptions:   
$$e_{ij}\sim N(0,\sigma_e)$$
<!-- $$(^{C_{i,0}}_{C_{i,1}}) \sim N_2(0,\sigma)$$ -->
$$\left(\begin{array}{c}
C_{i,0}\\
C_{i,1}\\
\end{array}\right) \sim N_2(0,\Sigma)$$ 

where $$\Sigma=\left[\begin{array}{c}
\sigma^2_{C_{i,0}}& \sigma_{(C_{i,0},C_{i,1})}\\
 \sigma_{(C_{i,0},C_{i,1})} & \sigma^2_{C_{i,1}}\\
\end{array}\right],$$
$$K=Cov(C_{i,0}, C_{i,1})$$
...
$$ln(\frac{Pr(healthy_i=1)}{Pr(healthy_i=0)})=\beta_0 + \beta_h + \beta_p$$
where   
...
```{r, eval=FALSE, echo=FALSE}
# table(cbpp$herd, cbpp$period)

# cbpp_mod <- cbpp %>% 
  # filter(herd != 8, herd!=2)
```
...
Model summary:
```{r, echo=FALSE}
summary(nonlin_bayes)
```

Posterior summary:
```{r, echo=FALSE}
posterior_summary(nonlin_bayes)
```

### Prediction
There are two ways to make predictions from a `brm` model. The first way is to use the generic `predict()` function that is built in. This function which will return an estimate using the fitted parameters from the model including the estimated error and quantiles.
...
The output represent the predicted number of healthy (non-diseased) cattle. 
...
head(preds)
```
You can also return the predictive probability by dividing by the "size" input. Predictive probability: 
$$pr(healthy)=\frac{exp(\hat\beta_0+\hat\beta_h+\hat\beta_p)}
{1+exp(\hat\beta_0+\hat\beta_h+\hat\beta_p)}$$
A simple way to show a histogram of the predictive probabilities is to use the `hist()` function:
...
In Bayesian context, we have the benefit of a distribution of estimates rather than a single point estimate. To view model estimates for each parameter, you can use the `as_draws()` function as we did earlier. The code below outputs $\hat\beta_0$ estimates from all post burn-in draws from chain 1:

```{r, eval=FALSE, results='hide'}
as_draws(nonlin_bayes)$`1`$b_Intercept
```

Using this function, we can create an dataframe of estimates for population slope $\hat\beta_0$, herd  specific random slope $\hat\beta_h$, and period specific random slope $\hat\beta_p$ for our earlier example of cattle in herd #2, period 4:
...
Vehtari A., Gelman A., Simpson D., Carpenter B., and Bürkner P. (2021). Rank-normalization, folding, and localization: An improved R-hat for assessing convergence of MCMC (with discussion). Bayesian Data Analysis.
