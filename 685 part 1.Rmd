...
$$ln(\frac{Pr(healthy_i=1)}{Pr(healthy_i=0)})=\beta_0 + \beta_{(0,herd)} + \beta_{(0,period)}$$
where   
* $\beta_0$ is the population intercept
* $\beta_{(0,h)}$ is is the random herd-level intercept for herd $h$
* $\beta_{(0,p)}$ is the random period-level intercept for period $p$

The herd level random effects are assumed to come from an iid normal distribution with mean 0 and some shared, herd-level variance, $\sigma^2_h \sim N(0,\sigma^2_h)$. Period-level random intercepts are assumed iid normal, mean 0 and shared period-level variance, $\sigma^2_p \sim N(0,\sigma^2_p)$

In examining the data, herd numbers 2 and 8 are both missing data. All other herds and periods have one observation. We will remove herds 2 and 8 and proceed with the resulting dataset. 
```{r}
table(cbpp$herd, cbpp$period)
```

```{r cbpp, echo = TRUE, results = 'hide'}
cbpp_mod <- cbpp %>% 
...
We will use priors obtained from the `glmer` model.
