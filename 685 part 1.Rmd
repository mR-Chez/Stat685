...
time_start <- Sys.time()
model1b_2k <- brm(Reaction ~ Days + (1 | Subject),
             data = sleepstudy,
             prior = priors,
             chains = 4,
             iter = 2000,
...
## Model Summary
With our model built, we can now get estimates of the parameters.

```{r}
summary(model1b)
```

The first 5 lines of the summary output describe our model and model inputs defined in the `brm` call. The next section shows the Group-Level effects. Our sd estimate for intercept is 61.02, with Est.Error of 10.58 which is an indication of the certainty we should have about the inference procedure. 
...
pop_slope <- coef(model1b)[["Subject"]][,,'Days'][1,1]
m1_coefs <- coef(model1b)[["Subject"]][,,'Intercept'][,1]
...
<!-- ## Model 2b -->
<!-- $$y_{ij} = \alpha_{intercept} + \beta*Days + \beta*C_{slope} + e_{residual}$$ -->
...
Now we build the model with random effects `Days` and `Subject` uncorrelated. 
...
model_list <- list(model1b, model3bc, model3bu)

add_crit <- function(model){
    model_out <- add_criterion(model,
                  c("waic", "loo", "bayes_R2"))
    return(model_out)
}

for(model in model_list){
    model <- add_crit(model)
}
