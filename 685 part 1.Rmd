...
$$P(y|\theta)$$
is known as the sampling distribution. Given this information, we can use our data (sampling distribution) along with a prior to calculate the posterior.

## The brm() function
This function in the *brms* package fits generalized linear/non-linear multivariate multilevel models (MMM) using Stan - a probabilistic programming language with a C++ backend. This function draws samples from the prior distribution, then combined with observed data calculates the posterior distribution.

## Model 1b 
In Model 1 above, our model is:
$$y_{Reaction} = \alpha_{intercept}  + C_{intercept} + \beta*Days + e_{residual}$$
## Priors
We need to choose prior distributions for parameters that might be helpful in forming a posterior distribution. Based on the exploratory data analysis done above, I will use parameters from our population data and fitted parameters from a linear model on the same data.

We need to first load in the *rstan* and *brms* packages which will allows us to build Bayesian models. To set priors, we use the *prior()* function, which accepts a distribution with parameters as well as a *class* argument which designates which parameter the prior is associated with.
...
Now we can get posterior estimates along with 95% credible intervals. The intercept estimates are shown as values in relation to the population-level effects (b_Intercept).
